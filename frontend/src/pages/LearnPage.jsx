...
export default function LearnPage() {
  ...
  async function downloadCertificate() {
    ...
    try {
      ...
      link.href = url;
      link.download = `${certificate.certificateNumber}.${extension}`;
      link.click();
      URL.revokeObjectURL(url);
    } catch (err) {
      setError(getApiErrorMessage(err) || "Failed to download certificate. Please try again.");
    } finally {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {course.enrollment.status === "COMPLETED" && (
        <div className="brand-gradient-subtle flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-[#7ABA78]/40 p-5 shadow-xs">
          <div>
            <div className="flex items-center gap-2">
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-600 text-white text-xs">
                🎓
              </span>
              <p className="font-bold text-[#063F2A] text-base">You have completed this course!</p>
            </div>
            <p className="mt-1 text-xs text-[#0A6847]/80">
              Your official Certificate of Completion has been verified and issued.
            </p>
          </div>
          {certificateQuery.data && (
            <div className="flex flex-wrap items-center gap-2.5">
              <button
                ...
                className="btn-primary-sm flex items-center gap-2 px-4 py-2 text-xs font-extrabold shadow-sm"
              >
              ...
              <button
                ...
                className="rounded-xl border border-emerald-300 bg-white px-3.5 py-2 text-xs font-bold text-[#0A6847] hover:bg-emerald-50 transition flex items-center gap-1.5 shadow-2xs disabled:opacity-50"
              >
                {isDownloading ? (
                  <>
      ...
      <CertificateModal
        ...
        onDownloadText={downloadCertificate}
        isDownloadingText={isDownloading}
      />
    </div>
  );
}
