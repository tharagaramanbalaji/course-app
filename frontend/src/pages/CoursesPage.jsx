...
export default function CoursesPage() {
  ...
  return (
    <div className="space-y-8">
            ...
            return (
              <motion.div variants={itemFadeUpVariants} key={course.id}>
                <CourseCard
                  ...
                  onPreview={() => setSyllabusCourse(course)}
                />
              </motion.div>
            );
          })}
      ...
      <CourseSyllabusModal
        isOpen={Boolean(syllabusCourse)}
        onClose={() => setSyllabusCourse(null)}
        course={syllabusCourse}
        isEnrolled={Boolean(syllabusCourse && myCoursesByCourseId.has(syllabusCourse.id))}
        onEnroll={(id) => enrollCourse.mutate(id)}
        enrolling={enrollCourse.isPending}
      />
    </div>
  );
...
function CourseCard({ course, myCourse, index, onEnroll, enrolling, onPreview }) {
  const style =
    CATEGORY_STYLES[course.category] ||
    PALETTE_FALLBACKS[index % PALETTE_FALLBACKS.length];
      ...
      <div className="flex flex-1 flex-col p-4 pt-3">
        ...
        <div className="mt-4 pt-3 border-t border-slate-100 space-y-2">
          <button
            type="button"
            onClick={onPreview}
            className="w-full inline-flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-100 hover:text-slate-900 transition"
          >
            <svg className="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
          ...
          {myCourse ? (
            <Link
              ...
              className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-[#0A6847] px-4 py-2 text-xs font-bold text-white shadow-xs hover:bg-[#085438] transition group/btn"
            >
            ...
            <button
              ...
              onClick={onEnroll}
              disabled={enrolling}
              className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-[#0A6847] px-4 py-2 text-xs font-bold text-white shadow-xs hover:bg-[#085438] transition disabled:opacity-50 group/btn"
            >
              <span>{enrolling ? "Enrolling..." : "Enroll & Start Course"}</span>
              <span className="transition-transform duration-200 group-hover/btn:translate-x-1.5">&rarr;</span>
            </button>
          ) : (
            <p className="text-center text-xs text-slate-500 py-1.5 font-medium bg-slate-50 rounded-xl border border-slate-100">
